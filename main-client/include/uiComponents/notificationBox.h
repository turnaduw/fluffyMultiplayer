...
/*
  void App::addNotification(int code)
  {
    FluffyMultiplayer::Notification tempNoti;

    switch (code)
    {
      case MS_RESPONSE_CONNECTION_ACCEPTED:
      {
        tempNoti.title = "connection";
        tempNoti.message = "connected to the server.";
        tempNoti.type = FluffyMultiplayer::NotificationType::Success;
      }break;
      case MS_ERROR_CONNECTION_REJECTED:
      case MS_ERROR_CONNECTION_EXISTS:
      {
        tempNoti.title = "connection";
        tempNoti.message = "could not connect to the server.";
        tempNoti.type = FluffyMultiplayer::NotificationType::Error;
      }break;

      case MS_ERROR_FAILED_TO_LOGIN_INCORRECT:
      case MS_ERROR_FAILED_TO_LOGIN_NOT_EXISTS:
      {
        tempNoti.title = "failed login";
        tempNoti.message = "incorrect username or password.";
        tempNoti.type = FluffyMultiplayer::NotificationType::Error;
      }break;

      case MS_ERROR_FAILED_TO_REGISTER_USERNAME_EXISTS:
      {
        tempNoti.title = "failed register";
        tempNoti.message = "username exists.";
        tempNoti.type = FluffyMultiplayer::NotificationType::Error;
      }break;

      case MS_ERROR_FAILED_TO_REGISTER_EMIAL_EXISTS:
      {
        tempNoti.title = "failed register";
        tempNoti.message = "email exists.";
        tempNoti.type = FluffyMultiplayer::NotificationType::Error;
      }break;


      case MS_ERROR_FAILED_TO_REGISTER_EASY_PASSWORD:
      {
        tempNoti.title = "failed register";
        tempNoti.message = "easy password.";
        tempNoti.type = FluffyMultiplayer::NotificationType::Error;
      }break;

      default:
      {
        std::cout <<"unkonwn code to add notification." << std::endl;
      }
    }
    if(!tempNoti.message.empty())
      notificationsQueue.push(tempNoti);
  }

  FluffyMultiplayer::Notification App::closeNotification()
  {
    FluffyMultiplayer::Notification noti = notificationsQueue.front();
    notificationsQueue.pop();
    return noti;
  }

  void App::closeNotification()
  {
    notificationsQueue.pop();
  }
